class_name GirlWithAnimationsSkin
extends CharacterSkin

## Sketchfab Spanish Maya rig (127 bones, FK/IK duplicates, has wings/tail/
## backpack). Ships with idle / walking / running / lanching ("launching" —
## reused as Jump + Dash). Same architecture as anime_character: damage-tint
## overlay, idle/attack/hit variant cycling (no-op with one clip each).

@export var extra_animation_sources: Array[PackedScene] = []

@export var skate_root_y: float = 0.134

const _FOOT_L_BONE := &"bn_L_tobillo_0104"
const _FOOT_R_BONE := &"bn_R_tobillo_0119"
...
func _ready() -> void:
	...
	_force_loop_linear(primary, ["idle", "walking", "running"])
