class_name Ch29NonPbrSkin
extends CharacterSkin

## Mixamo "Ch29_nonPBR" character (65 bones, mixamorig1_* naming — note the
## numeric suffix on the namespace, distinct from Aj's mixamorig_*).
## Animation library is the same 15 Mixamo anims + Ch29's bind-pose action,
## merged via tools/import_mixamo.py with hip-translation tracks stripped.
## Single-mesh body (just `Ch29`), so the damage tint paints one mesh only.

@export var extra_animation_sources: Array[PackedScene] = []

@export var skate_root_y: float = 0.134

const _FOOT_L_BONE := &"mixamorig1_LeftFoot"
const _FOOT_R_BONE := &"mixamorig1_RightFoot"
