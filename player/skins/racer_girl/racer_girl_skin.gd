class_name RacerGirlSkin
extends CharacterSkin

## Sketchfab 3ds Max Biped rig (54 bones). Ships with only Idle + Walking —
## every action state (Jump/Fall/Dash/Attack/EdgeGrab/WallSlide/Crouch) falls
## back to one of those two clips. Same architecture as anime_character:
## damage-tint overlay flushes the body mesh red on hit; idle/attack/hit
## variant cycling is wired but no-ops with only one clip per slot.

@export var extra_animation_sources: Array[PackedScene] = []

@export var skate_root_y: float = 0.134

const _FOOT_L_BONE := &"Bip001 L Foot_051"
const _FOOT_R_BONE := &"Bip001 R Foot_047"
...
func _ready() -> void:
	...
	_force_loop_linear(primary, ["Idle", "Walking"])
