class_name AnimeCharacterSkin
extends CharacterSkin

## Anime-style mannequin skin (UAL-compatible bones: foot_l / foot_r). All 24
## clips ship inside the single rig GLB — no extra_animation_sources needed.
## Conforms exactly to the CharacterSkin contract.

## Extra GLBs whose animations get merged into the primary AnimationPlayer
## at _ready. Empty for this skin (clips are baked into the model GLB).
...
# Cached Dash node — the rig has a single non-directional Roll clip.
var _dash_anim_node: AnimationNodeAnimation

# Attack alternates between the two Fighting Jab clips per swing.
var _edge_anim_node: AnimationNodeAnimation
const _ATTACK_CLIPS := [&"Fighting Left Jab", &"Fighting Right Jab"]
...
const _IDLE_CLIPS := [&"Idle", &"Idle_Talking", &"Idle Listening"]
var _idle_cycle_index: int = 0

# Damage overlay applied to the rig's two skinned meshes.
...
func _ready() -> void:
	...
	# GLB imports default to LOOP_NONE — patch loopable clips. One-shots
	# (Jump_Start, Jump_Land, Roll, attacks, hits, Death01, etc.) stay
	# LOOP_NONE so they fire and resolve cleanly.
	_force_loop_linear(primary, [
		"Idle", "Idle_Talking", "Idle Listening",
		"Walk", "Jog_Fwd", "Run Anime",
		"Crouch_Fwd",
