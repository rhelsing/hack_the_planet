class_name HandpaintGirlSkin
extends CharacterSkin

## Sketchfab Maya-joint rig (99 bones, "girl02_*" naming). Ships with a
## single IDLE clip — every state in the AnimationTree falls back to IDLE
## until more clips are added. The GLB also bakes in a Plane001_GROUND
## floor plane that we hide at runtime so the character isn't standing on
## a duplicate of the world ground.

@export var extra_animation_sources: Array[PackedScene] = []

@export var skate_root_y: float = 0.134

const _FOOT_L_BONE := &"girl02_foot_l_080"
const _FOOT_R_BONE := &"girl02_foot_r_086"
...
const _ATTACK_CLIPS := [&"IDLE"]

var _hit_anim_node: AnimationNodeAnimation
const _HIT_CLIPS := [&"IDLE"]
...
func _ready() -> void:
	_hide_packaged_ground_plane(self)
	...
	_force_loop_linear(primary, ["IDLE"])
...
# The GLB ships with a "Plane001" ground plane authored alongside the
# character. Hide every node whose name begins with "Plane001" so it
# doesn't render on top of (or below) the world floor.
func _hide_packaged_ground_plane(n: Node) -> void:
	if n.name.begins_with("Plane001") and n is Node3D:
		(n as Node3D).visible = false
		return
	for c: Node in n.get_children():
		_hide_packaged_ground_plane(c)


# Skip the ground plane subtree so the damage tint isn't painted onto it
# (and so we don't grab it back after _hide_packaged_ground_plane ran).
func _collect_mannequin_meshes(n: Node) -> void:
	if n.name.begins_with("Plane001"):
		return
