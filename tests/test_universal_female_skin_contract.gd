...
## Proves universal_female_skin.tscn conforms to CharacterSkin and that the
...
func _init() -> void:
	var scene: PackedScene = load("res://player/skins/universal_female/universal_female_skin.tscn")
	if scene == null:
		_failures.append("could not load universal_female_skin.tscn")
		_finish()
		return

	_skin = scene.instantiate()
	if not (_skin is CharacterSkin):
		_failures.append("universal_female scene root doesn't extend CharacterSkin (got %s)" % _skin.get_class())
		_finish()
		return
	...
	for m: String in ["idle", "move", "fall", "jump", "edge_grab", "wall_slide", "attack", "dash", "crouch", "die", "land", "on_hit"]:
		if not _cs.has_method(m):
			_failures.append("universal_female missing contract method: %s" % m)
	_cs.damage_tint = 5.0
...
func _on_skin_ready() -> void:
	...
	if anim == null:
		_failures.append("universal_female skin has no AnimationPlayer")
	else:
		# Clips referenced by the AnimationTree — must exist in the merged library.
		var expected := [
			"Idle",             # Idle state
			"Idle_FoldArms",    # Idle variant B
			"Sprint",           # Move (Run)
			"Jog_Fwd",          # Move strafe fallback
			"Punch_Jab",        # Attack
			"Punch_Cross",      # Attack variant
			"Jump_Start",       # Jump
			"Jump",             # Fall
			"Jump_Land",        # Land
			"Roll",             # Dash
			"Crouch_Idle",      # Crouch
			"Crouch_Fwd",       # WallSlide (fallback-sneak)
			"Death01",          # Die
			"Hit_Chest",        # Hit
			"Hit_Head",         # Hit variant
		]
...
func _finish() -> void:
	...
	if _failures.is_empty():
		print("PASS test_universal_female_skin_contract: UniversalFemale conforms to CharacterSkin")
		quit(0)
	else:
		for f: String in _failures:
			printerr("FAIL test_universal_female_skin_contract: " + f)
		quit(1)
