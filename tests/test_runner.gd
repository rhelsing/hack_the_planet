...
func _run_save_service() -> void:
	...
	# begin_new_game sets active_slot and writes the slot.
	...
	# clear_active_slot drops the binding.
	...
	var pre_count := ss.playtime_s
	ss._on_checkpoint_reached(Vector3.ZERO)
	# (No assertion besides "doesn't crash" — reaching here is the test.)
	...
	# Scratch slot save + load round-trip, backing up any real file.
	var scratch: StringName = &"a"
	var save_path: String = ss._save_path(scratch)
	var meta_path: String = ss._meta_path(scratch)
	var had_save := FileAccess.file_exists(save_path)
	var had_meta := FileAccess.file_exists(meta_path)
	if had_save: DirAccess.rename_absolute(save_path, save_path + ".runner_bak")
	if had_meta: DirAccess.rename_absolute(meta_path, meta_path + ".runner_bak")

	_expect("save_service", "has_slot empty after backup", not ss.has_slot(scratch))
	ss.current_level = &"runner_level"
	ss.playtime_s = 77.7
	ss.save_to_slot(scratch)
	_expect("save_service", "has_slot true after save_to_slot", ss.has_slot(scratch))
	_expect("save_service", "save file written", FileAccess.file_exists(save_path))
	_expect("save_service", "meta file written", FileAccess.file_exists(meta_path))
	var meta: Dictionary = ss.slot_metadata(scratch)
	_expect("save_service", "metadata has timestamp", meta.has("timestamp"))
	_expect("save_service", "metadata level_id matches",
		String(meta.get("level_id", "")) == "runner_level")

	# Full-save JSON shape.
